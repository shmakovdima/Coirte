...
switch($_POST['action']) {
    ...
    case 'delete_quest':
        ...
        if(isset($_POST['front'])){
            $front = (int) $_POST['front'];
        }

        $sql = "DELETE FROM `".$table_quest."` WHERE `id` = ".$id;
        $results = $wpdb->query( $sql );
        
        if(isset($front) and is_int($front) and $front == 1){
    ...
    case 'update_quest':
        ...
        $id_book = (int) $_POST['id_book']; if(!is_int($id_book)){ break; }
    ...
    case 'change_faqdisplay':
        ...
        $mode = (int) $_POST['mode']; if(!is_int($mode)){ break; }
